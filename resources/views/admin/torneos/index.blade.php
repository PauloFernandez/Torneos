<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <form action="{{ route('torneos.index') }}" method="get" class="flex-1 sm:flex-initial">
            @csrf
                <div class="flex gap-2">
                    <input class="flex-1 sm:w-64 px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all"
                           type="text" name="search" value="{{ old('search') }}" placeholder="Buscar torneos...">
        ...
        <x-slot name="body">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-6 whitespace-nowrap text-sm font-medium flex">
                    @can('Editar Torneo')
                    <a href="{{ route('torneos.edit', $torneo) }}" class="text-blue-600 hover:text-blue-900 mr-3">
