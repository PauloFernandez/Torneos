<x-app-layout>
    ...
    <form action="{{ route('partidos.store') }}" method="POST" class="space-y-4" enctype="multipart/form-data">
        @csrf
        <!-- Información básica -->
        ...
        <div class="flex gap-4 mb-4">
            ...
            <div class="flex-1">
                ...
                <select name="arbitro_id" id="arbitro_id"
                    ...
                    <option selected disabled>Seleccionar arbitro</option>
                    @foreach ($arbitros as $arbitro)
                        <option value="{{ $arbitro->id }}" {{ old('arbitro_id') == $arbitro->id ? 'selected' : '' }}>
                            {{ $arbitro->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="equipo_local_id" id="equipo_local_id"
                    ...
                    <option selected disabled>Seleccionar equipo local</option>
            ...
            <div>
                ...
                <select name="equipo_visitante_id" id="equipo_visitante_id"
                    ...
                    <option selected disabled>Seleccionar equipo visitante</option>
                    {{-- Opciones se llenarán dinámicamente con JavaScript --}}
                </select>
    ...
    <script>
        ...
        function populateEquipos(selectedTorneoId, oldEquipoLocalId = null, oldEquipoVisitanteId = null) {
            ...
            equipoLocalSelect.innerHTML = '<option selected disabled>Seleccionar equipo local</option>';
            equipoVisitanteSelect.innerHTML = '<option selected disabled>Seleccionar equipo visitante</option>';

            if (!selectedTorneoId) {
                return; // Si no hay torneo seleccionado, no hacemos nada más.
            }
            ...
            filteredEquipos.forEach(equipo => {
                ...
                // Si hay un valor old y coincide, lo selecciona
                if (oldEquipoLocalId && equipo.id == oldEquipoLocalId) {
                ...
                // Si hay un valor old y coincide, lo selecciona
                if (oldEquipoVisitanteId && equipo.id == oldEquipoVisitanteId) {
        ...
        const oldEquipoLocalId = "{{ old('equipo_local_id') }}" || null;
        const oldEquipoVisitanteId = "{{ old('equipo_visitante_id') }}" || null;

        if (initialTorneoId) {
            populateEquipos(initialTorneoId, oldEquipoLocalId, oldEquipoVisitanteId);
        }
