<x-app-layout>
    <div class="text-center mb-9 mt-5">
        <h1 class="text-3xl font-bold text-gray-800 mb-2">Actualizar Tarjeta</h1>
    </div>

    <form action="{{ route('tarjetas.update', $tarjeta) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        <!-- Información básica -->
        <div class="grid grid-cols-2 gap-4">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="nombre" placeholder="Indique el nombre de la tarjeta"
                     class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500
                        form-control @error('nombre') is-invalid @enderror is-valid" value="{{ old('nombre', $tarjeta->nombre) }}">
                </div>
                @error('nombre')
            ...
            <div>
                <label for="multa" class="block text-sm font-medium text-gray-700 mb-1">Consto por hora</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <i class="fas fa-dollar-sign text-gray-400"></i>
                    </div>
                    <input type="number" name="multa" value="{{ old('multa', $tarjeta->multa) }}"
                        class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 
