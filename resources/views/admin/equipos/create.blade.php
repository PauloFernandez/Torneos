<x-app-layout>
    ...
    <form action="{{ route('equipos.store') }}" method="POST" class="space-y-4" enctype="multipart/form-data">
        @csrf
        <!-- Foto de perfil -->
        <div class="mb-4">
            ...
            <div>
                ...
                <select name="torneo_id" id="torneo_id"
                    ...
                    @foreach ($torneos as $torneo)
                        <option value="{{ $torneo->id }}" id="{{ $torneo->inscripcion }}">{{ $torneo->nombre }}</option>
                    @endforeach
                </select>
                 @error('torneo_id')
                    <span class="invalid-feedback text-red-600">
